...
def measurement_update(sensor_var, p_cov_check, y_k, p_check, v_check, q_check):
    ...
    covar = (np.eye(3) * sensor_var)
    kalman_gain = p_cov_check.dot(h_jac.T).dot(np.linalg.inv(h_jac.dot(p_cov_check).dot(h_jac.T) + covar))
    # 3.2 Compute error state
    err_state = (kalman_gain.dot(y_k - p_check))
    # 3.3 Correct predicted state
    p_hat = p_check + err_state [0:3]
    v_hat = v_check + err_state [3:6]
    q_hat = Quaternion(euler=err_state [6:9]).quat_mult_left(q_check)
